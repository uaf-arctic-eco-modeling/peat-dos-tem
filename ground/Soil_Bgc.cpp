...
//	cout<<ed->d_soid.ch4[0]<<"\t"<<ed->d_soid.ch4[1]<<"\t"<<ed->d_soid.ch4[2]<<"\t"<<ed->d_soid.ch4[3]<<"\t"<<ed->d_soid.ch4[4]<<"\t"<<ed->d_soid.ch4[5]<<"\t"<<ed->d_soid.ch4[6]<<"\t"<<ed->d_soid.ch4[7]<<"\t"<<ed->d_soid.ch4[8]<<"\t"<<ed->d_soid.ch4[9]<<"\t"<<ed->d_soid.ch4[10]<<"\n"<<endl;
    ...
    for (il = 0; il < numsl; il++)
    {
        ...
        {
        if (ed->d_soid.ch4[il]>0.8)
            ed->d_soid.ch4[il]=0.8;
        else if (ed->d_soid.ch4[il]<0.00000001)
                ed->d_soid.ch4[il]=0.0;
        }
        
        else
            
        {
        if (ed->d_soid.ch4[il]>8) 
            ed->d_soid.ch4[il]=8;
        else if (ed->d_soid.ch4[il]<0.00000001)
                ed->d_soid.ch4[il]=0.0;
        }
...
//	cout<<ed->d_sois.ch4[0]<<"\t"<<ed->d_sois.ch4[1]<<"\t"<<ed->d_sois.ch4[2]<<"\t"<<ed->d_sois.ch4[3]<<"\t"<<ed->d_sois.ch4[4]<<"\t"<<ed->d_sois.ch4[5]<<"\t"<<ed->d_sois.ch4[6]<<"\t"<<ed->d_sois.ch4[7]<<"\t"<<ed->d_sois.ch4[8]<<"\t"<<ed->d_sois.ch4[9]<<"\t"<<ed->d_sois.ch4[10]<<"\n"<<endl;
	...
	for (j = 1; j <= m; j++)
    {
		...
		for (il = numsl - 1; il > 0; il--)
        {
			...
			Plant = bd->rp * ed->m_sois.rootfrac[il] * ed->d_soid.ch4[il] * bd->tveg * ed->d_vegs.realLAI * 0.5; //bd->m_vegd.lai / 2.0;
                ...
                
            {
                ...
                if (ed->d_sois.ts[il] >= -2.0 && ed->d_sois.ts[il] < 0.000001) TResp = getRhq10(-6.0) * 0.25;
                else if (ed->d_sois.ts[il] < -2.0) TResp = 0.0;
                
                if (tmp_sois.reac[il] > 0) del_soi2a.rrh_m[il] = klitrc * tmp_sois.reac[il] * TResp;
                else del_soi2a.rrh_m[il] = 0.0;
               ...
               //  cout<<"Eb="<<Ebul<<"\n"<<endl;
                ...
                if (ed->d_sois.ts[il] > 0.0) Plant_m = Plant * ed->m_sois.por[il] * ed->m_sois.dz[il] * 1000.0;
                else Plant_m = Plant * ed->d_soid.alllwc[il] * ed->m_sois.dz[il] * 1000.0;
            }
        ...
        for (il = 1; il < numsl; il++) ed->d_soid.ch4[il] = V[il];
       
        tmp_flux = diff[1] * (ed->d_soid.ch4[1] - ub) / ed->m_sois.dz[1];
		
        if (tmp_flux < 0.0000001) tmp_flux = 0.0;
	...
	if (ed->d_soid.watertab < 0.0000001) totFlux_m = 0.5 * totPlant_m + Flux2A_m + totEbul_m;
	else totFlux_m = 0.5 * totPlant_m + Flux2A_m;

	if (totFlux_m > 0) ed->d_soid.dfratio = Flux2A_m / totFlux_m * 100.0;
    else ed->d_soid.dfratio = 0.0;
